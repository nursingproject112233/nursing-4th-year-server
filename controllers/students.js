...
export const getStudents = async (req, res) => {
  try {
    ...
    if (user_data.type == "student") {
      ...
      const { start, limit, term, year } = req.query;
  ...
  try {
    ...
    if (user_data.type == "student") {
      ...
      if (!mongoose.Types.ObjectId.isValid(id))
        return res
          .status(404)
          .json({ message: `No entry found with id: ${id}` });

      const studentrec = await Student.findByIdAndUpdate(id, body, {
        new: true,
      });
...
export const registerToHospital = async (req, res) => {
  try {
    ...
    if (toggleObj.hospital === false) {
      return res
        .status(400)
        .json({ message: "Hospital registration not open yet" });
    }

    const student = await Student.findOne({
      email: req.user.username,
    }).populate("placements.agency");
    ...
    for (let i = 0; i < student.placements.length; i++) {
      if (student.placements[i].agency.agency_type === agency_type) {
        return res
          .status(400)
          .json({ message: "Already registered for a Hospital!" });
      }
    }
...
export const registerToCommunity = async (req, res) => {
  try {
    ...
    if (toggleObj.community === false) {
      return res
        .status(400)
        .json({ message: "Community registration not open yet" });
    }
    ...
    for (let i = 0; i < student.placements.length; i++) {
      if (student.placements[i].agency.agency_type === agency_type) {
        return res
          .status(400)
          .json({ message: "Already registered for a Community!" });
      }
    }
    const agency = await Agency.findOne({
      name: agency_name,
  ...
  try {
    ...
    if (user.type == "student") {
      ...
      const student = await Student.findOne(
        ...
        .sort({ createdAt: -1 })
        .populate({
          path: "placements.agency",
          select: ["name", "placement_type", "agency_type"],
        });
      const total_count = student.placements.length;
      if (total_count == 0) {
        return res
          .status(200)
          .json({ student, message: "No placements", total_count });
      }
      return res
        .status(200)
        .json({
    ...
    if (!id && user.type == "admin") {
      ...
      return res
        .status(200)
        .json({ student, msg: "list of Student-Placement details" });
    }
...
export const placementTypes = async (req, res) => {
  try {
    ...
    const placement_types = await Agency.find(
      { agency_type },
      "placement_type"
    ).distinct("placement_type");
    return res
      .status(200)
      .json({ placement_types, msg: "List of placement-types" });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
};
...
export const agencyNames = async (req, res) => {
  try {
    ...
    if (total_count == 0) {
      return res
        .status(200)
        .json({ agency, total_count, message: "No agencies available" });
    }
    return res
      .status(200)
      .json({
        agency,
        total_count,
        msg: "List of agency-names with slots available",
      });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
