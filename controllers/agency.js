  ...
  try {
    ...
    if (user_details.type == "student") {
      agencies = await Agency.find(
        { visibility: true, agency_type: agency_type },
        { _id: 1, name: 1, placement_type: 1 }
      ).sort({ createdAt: -1 });
    } else {
      agencies = await Agency.find(
...
export const deleteAgency = async (req, res) => {
  try {
    ...
    await Student.updateMany(
      { "placements.agency": id },
      { $pull: { placements: { agency: id } } },
      { new: true }
    );
...
export const agencyPlacements = async (req, res) => {
  try {
    ...
    const agency = await Agency.findById(id, "name placement_type agency_type")
      .sort({ createdAt: -1 })
      .populate({
        path: "placements.student",
        select: ["studentId", "fname", "lname", "email", "year", "term"],
      });
...
export const toggleAgencyRegistration = async (req, res) => {
  try {
    ...
    const toggleObj = await ToggleAgencyRegister.findOneAndUpdate(
      {},
      req.body,
