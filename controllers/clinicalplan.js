...
export const getClinicalPlans = async (req, res) => {
  try {
    ...
    if (user.type == "student") {
      ...
      const clinical_plans = await ClinicalPlan.find({ studentId }).sort({
        createdAt: -1,
      });
    ...
    const clinical_plans = await ClinicalPlan.find({}).sort({ createdAt: -1 });
...
export const getSemOptions = async (req, res) => {
  try {
    // list of options
    const semesterOptions = await SemesterOptions.find({}).sort({
      createdAt: -1,
    });
...
export const setSemOptions = async (req, res) => {
  try {
    ...
    if (user.type == "student") {
      return res
        .status(400)
        .json({ message: "Only Admins can edit a Clinical Plan." });
    }

    // reset the available seats for each semester option
    if (req.body.reset_seats_filled === true) {
      const semesterOption = await SemesterOptions.updateMany(
        {},
        { seats_filled: 0 },
      ...
      return res.status(200).json({
        data: all_options,
        message: "Seats availability reset successfull.",
      });
    }
    ...
    for (let i in options) {
      const semesterOption = await SemesterOptions.findOne({
        field: options[i],
      });
      let seats = req.body[options[i]];
      if (!semesterOption) {
        const new_option = new SemesterOptions({
          field: options[i],
          seats: seats,
        });
    ...
    return res.status(200).json({
      data: all_options,
      message: "Semester Options updated successfully.",
    });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
...
export const delSemOptions = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Semester Options deleted successfully." });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
};
...
export const addClinicalPlan = async (req, res) => {
  try {
    ...
    if (toggleObj.registration_open === false) {
      return res
        .status(400)
        .json({ message: "ClinicalPlan registration not open yet" });
    }
    const user = req.user;
    if (user.type != "student") {
      return res
        .status(400)
        .json({ message: "Only Students can register for a Clinical Plan!" });
    } else {
      // fetching studentId
      const student_details = await Student.findOne({ email: user.username });
      let studentId = student_details.studentId;
      ...
      const semesterOptions = await SemesterOptions.find({
        semester_sequence: req.body.semester_sequence,
      });
...
export const toggleClinicalPlanRegistration = async (req, res) => {
  try {
    ...
    const toggleObj = await ToggleClinicalPlanRegister.findOneAndUpdate(
      {},
      req.body,
      { returnOriginal: false }
    );
    ...
    return res
      .status(200)
      .json({ data: toggleObj, message: "Updated Successfully" });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
};
